...
!
  ...
  character ( len = 255 ) :: perm_rcm_filename = ' '
...
!
  ...
  perm_rcm_filename = trim (prefix) // '_rcm_perm.txt'
!
!  Read the node data.
!
...
!  Write the permutation
  call rcm_permutation_write ( perm_rcm_filename, node_num, perm)
!
...
subroutine rcm_permutation_write ( output_filename, n, perm)
...
!
  ...
  if ( output_status /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'PERM_WRITE - Fatal error!'
    write ( *, '(a,i8)' ) '  Could not open the output file "' // &
      trim ( output_filename ) // '" on unit ', output_unit
    output_unit = -1
    stop
...
!                                            '(', m, 'g', 24, '.', 16, ')'
!
!  write ( string, '(a1)' ) '(', m, 'g', 14, '.', 6, ')'
!
!  Write the data.
!
  do j = 1, n
    write ( output_unit, *) perm(j)
  end do
!
!  Close the file.
!
