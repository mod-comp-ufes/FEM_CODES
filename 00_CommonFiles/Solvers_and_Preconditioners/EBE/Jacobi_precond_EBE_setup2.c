...
int Jacobi_precond_EBE_setup2 (ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs, int tag, double *F)
{
	int I, E, K;
	...
	invDiag[neq] = 1.0;
	for (I=0; I<nel; I++){
		MatrixData->invDe[I][0] = 1.0 / (invDiag[lm[I][0]]*A[I][0]);
		MatrixData->invDe[I][1] = 1.0 / (invDiag[lm[I][1]]*A[I][4]);
		MatrixData->invDe[I][2] = 1.0 / (invDiag[lm[I][2]]*A[I][8]);
	...
	/* F preconditioning */
	double *faux = mycalloc("faux of Jacobi_precond_EBE_setup2",(neq + 1), sizeof(double));
	for (I = 0; I < neq; I++){
		faux[I] = F[I];
	}
	faux[neq] = 0.0;
