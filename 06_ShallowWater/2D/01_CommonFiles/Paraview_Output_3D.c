...
int Paraview_Output_3D(ParametersType *Parameters, FemStructsType *FemStructs, FemFunctionsType *FemFunctions)
{
	...
	h = (double*) mycalloc("h of 'Paraview_Output_3D'", nnodes, sizeof(double));
	qx = (double*) mycalloc("qx of 'Paraview_Output_3D'", nnodes, sizeof(double));
	qy = (double*) mycalloc("qy of 'Paraview_Output_3D'", nnodes, sizeof(double));
	...
	sprintf(FileName,"%s%s_%s_%s_%s_%s_%s_%s_N%d_E%d_3D.vtu", Parameters->outPath, Parameters->Experiments, Parameters->ProblemTitle, Parameters->StabilizationForm, Parameters->ShockCapture,
