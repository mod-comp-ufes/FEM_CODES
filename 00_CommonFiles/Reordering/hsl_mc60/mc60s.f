...
C Version 1.1.0. For history see ChangeLog
      ...
      SUBROUTINE MC60A(N,LIRN,IRN,ICPTR,ICNTL,IW,INFO)
...
C    needed in later cycles of the loop.
   ...
   80 CONTINUE
      ...
      SUBROUTINE MC60B(N,LIRN,IRN,ICPTR,NSUP,SVAR,VARS,IW)
...
C     .. External Subroutines ..
      EXTERNAL MC60O,MC60P

C Look for supervariables
      FLAG = N+1
      CALL MC60O(N,N,LIRN,IRN,ICPTR,SVAR,NSUP,IW,VARS,IW(FLAG))

C Adjust the permutation and compress the pattern
      CALL MC60P(N,NSUP,LIRN,IRN,ICPTR,SVAR,VARS,IW,IW(FLAG))

      END

      SUBROUTINE MC60C(N,NSUP,LIRN,IRN,ICPTR,VARS,JCNTL,
...
C       Find pseudoperipheral pair of nodes for this component
          CALL MC60H(N,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,
...
C Pseudoperipheral pair specified. Try first end.
          NSTOP = PAIR(1,I)
          CALL MC60L(NSTOP,N,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,
...
C Find max. and min. given priority value
          ...
          DO 10 J = 1,NSUP
            ...
            END IF
   10     CONTINUE
          CALL MC60L(NSTRT,N,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,
     +            IW(XLS+1),NLVL,LWIDTH,NVARS)
          NODES = IW(XLS+NLVL+1)-1
...
C degree (last time level set constructed it was with NSTOP as root)
           CALL MC60Q(NSTOP,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,
     +            IW(XLS+1),NLVL,NVARS)
           DO 11 J = NODES,1,-1
             LSTNUM = LSTNUM + 1
             PERMSV(IW(J)) = -LSTNUM
...
C         Order nodes in this component
          CALL MC60J(NSUP,LIRN,NODES,NSTRT,LSTNUM,IRN,ICPTR,
...
C     Set new node numbers to +ve values
   ...
   40 CONTINUE
      ...
      SUBROUTINE MC60D(N,NSUP,SVAR,VARS,PERMSV,PERM,POSSV)
...
C Construct full permutation
   ...
   30 CONTINUE
      ...
      SUBROUTINE MC60E(N,NSUP,LIRN,IRN,ICPTR,SVAR,VARS,PERMSV,PERM,IW)
...
C Construct full presentation order
   ...
   50 CONTINUE
      ...
      SUBROUTINE MC60F(N,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,RINFO)
...
C Update statistics
         RINFO(3) = MAX(RINFO(3),REAL(IW(NSUP+I+1)-IW(NSUP+IMIN)))
         IF (IW(J).GT.0) THEN
C J not yet in front. Treat is diagonal block as a diagonal matrix.
           IW(J) = -IW(J)
           RINFO(2) = MAX(RINFO(2),REAL(NACTIV+1))
           RINFO(1) = RINFO(1) + NV*REAL(NACTIV+1)
           RINFO(4) = RINFO(4) + NV*REAL(NACTIV+1)**2
         ELSE
           RINFO(2) = MAX(RINFO(2),REAL(NACTIV))
           DO 70 J = 1, NV
             RINFO(1) = RINFO(1) + REAL(NACTIV)
             RINFO(4) = RINFO(4) + REAL(NACTIV)**2
C Remove node from the front
   ...
   80 CONTINUE
      ...
      RINFO(4) = SQRT(RINFO(4)/REAL(N))

      END



      SUBROUTINE MC60G(N,NSUP,LIRN,IRN,ICPTR,VARS,PERMSV,IW,RINFO)
...
C Update row/column front sizes.
         RINFO(1) = MAX(RINFO(1),REAL(KFRNT))
         RINFO(2) = MAX(RINFO(2),REAL(LFRNT))
         IF (-IW(I).EQ.IEQ) THEN
C Supervariable I can be eliminated.
            IW(I) = 0
            NV = VARS(I)
            DO 65 K = 1,NV
               RINFO(3) = RINFO(3) + REAL(KFRNT)**2
               RINFO(4) = RINFO(4) + REAL(KFRNT)*REAL(LFRNT)
               LFRNT = LFRNT - 1
               KFRNT = KFRNT - 1
   65       CONTINUE
         END IF
...
C Compute r.m.s row front size.
      RINFO(3) = SQRT(RINFO(3)/REAL(N))
C Compute mean frontal matrix size.
      RINFO(4) = RINFO(4)/REAL(N)
      ...
      SUBROUTINE MC60H(N,NSUP,LIRN,IRN,ICPTR,VARS,MASK,LS,XLS,LIST,
     +                 INFO)
...
C     Generate level structure for node with min supervariable degree
      CALL MC60L(NSTRT,N,NSUP,LIRN,IRN,ICPTR,VARS,MASK,LS,XLS,
     +            MAXDEP,LWIDTH,NVARS)
...
C  Form rooted level structures NODE
          CALL MC60L(NODE,MINWID,NSUP,LIRN,IRN,ICPTR,VARS,MASK,LS,XLS,
     +               NLVL,LWIDTH,NVARS)
...
C Swap the ends if this reduces the width
   80 IF (INFO(4) .LT. MINWID) THEN
      ...
      IF(NSTOP.NE.NODE) CALL MC60L(NSTOP,N,NSUP,LIRN,IRN,ICPTR,VARS,
     +               MASK,LS,XLS,NLVL,LWIDTH,NVARS)
      ...
      SUBROUTINE MC60J(NSUP,LIRN,NODES,NSTRT,LSTNUM,IRN,ICPTR,VARS,
...
C Adjust heap
  ...
  170 CONTINUE
      ...
      SUBROUTINE MC60L(ROOT,MAXWID,NSUP,LIRN,IRN,ICPTR,VARS,MASK,LS,
...
C     Reset MASK for nodes in the level structure
   ...
   50 CONTINUE
      ...
      SUBROUTINE MC60O(N,NC,LIRN,IRN,ICPTR,SVAR,NSUP,NEW,VARS,FLAG)
...
C Subsequent occurrence of IS for column J
   ...
   40 CONTINUE
      ...
      SUBROUTINE MC60P(N,NSUP,LIRN,IRN,ICPTR,SVAR,VARS,VAR,FLAG)
...
C Do actual compression
   ...
   60 CONTINUE
      ...
      SUBROUTINE MC60Q(ROOT,NSUP,LIRN,IRN,ICPTR,VARS,MASK,LS,
     +                 XLS,NLVL,NVARS)
...
C     .. Local Scalars ..
      INTEGER FNBR,I,J,K,L,LBEGIN,LNBR,LPERM,LVLEND,LW,NBR,NODE
