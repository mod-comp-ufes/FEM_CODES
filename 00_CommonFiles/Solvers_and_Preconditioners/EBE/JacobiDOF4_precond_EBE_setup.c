...
int JacobiDOF4_precond_EBE_setup (ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs, int tag, double *F)
{
	int I;
	...
	for (I = 0; I < nel; I++){
		...
		MatrixData->invDe[I][1] = 1.0/(MatrixData->A[I][13]);
		MatrixData->invDe[I][2] = 1.0/(MatrixData->A[I][26]);
		MatrixData->invDe[I][3] = 1.0/(MatrixData->A[I][39]);

		MatrixData->invDe[I][4] = 1.0/(MatrixData->A[I][52]);
		MatrixData->invDe[I][5] = 1.0/(MatrixData->A[I][65]);
		MatrixData->invDe[I][6] = 1.0/(MatrixData->A[I][78]);
		MatrixData->invDe[I][7] = 1.0/(MatrixData->A[I][91]);

		MatrixData->invDe[I][8] = 1.0/(MatrixData->A[I][104]);
		MatrixData->invDe[I][9] = 1.0/(MatrixData->A[I][117]);
		MatrixData->invDe[I][10] = 1.0/(MatrixData->A[I][130]);
		MatrixData->invDe[I][11] = 1.0/(MatrixData->A[I][143]);
	...
	/* F preconditioning */
	double *faux = mycalloc("faux of JacobiDOF4_precond_EBE_setup",(neq + 1), sizeof(double));
	for (I = 0; I < neq; I++){
		faux[I] = F[I];
	}
	faux[neq] = 0.0;
	...
	JacobiDOF4_precond_EBE (Parameters, MatrixData, FemStructs, faux, F);
