...
int main(int argc, char **argv)
{
	...
	double xInf, yInf, xSup, ySup, hx, hy, Y, X, cy, cx, r, yfixo;
	...
	for (j = 0; j < nosy; j++){
		if(j==0){
			yfixo = yInf;
				...
				printf("\n Valor de k = %d \n", k);
				yfixo = coord[k][1] + 0.0115845*pow(r,(j-1));
		}else{
				yfixo = coord[k][1] + 0.0115845*pow(r,(40-j));
		}
		// 
		//coord[k][0] = xInf;
		//
		for (i = 0; i < nosx; i++){
			if(i==0){
				...
				k++;			
				coord[k][0] = coord[k-1][0] + coord[1][0]*pow(r,(i-1));
				coord[k][1] = yfixo;				
			}else{
				k++;			
				//printf("\n Valor de i = %d \n", i);
				coord[k][0] = coord[k-1][0] + coord[1][0]*pow(r,(40-i));
				coord[k][1] = yfixo;
			} 
		}
	}
