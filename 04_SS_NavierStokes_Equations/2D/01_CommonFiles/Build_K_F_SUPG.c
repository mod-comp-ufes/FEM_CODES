...
int Build_K_F_SUPG(ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs, FemFunctionsType *FemFunctions)
{
	...
	//double Uref, Lref;
	double Re, epsilon;
	double twoArea, invArea, Area, tau, tau_PSPG, h, visc, rho, invrho, nu, unorm, auxtau, tau_l; 
	...
	double Nphi11, Nphi12, Nphi13, Nphi21, Nphi22, Nphi23, Nphi31, Nphi32, Nphi33, Nphi41, Nphi42, Nphi43, Nphi51, Nphi52, Nphi53, Nphi61, Nphi62, Nphi63;
	...
	for (E=0; E<nel; E++)
	{
		...
		//h = sqrt( 4*Area/PI ); // Dissertacao Renato Elias	
		h=sqrt(twoArea);		
		visc =1./Re;
		rho = 1.;		
		...
		//	printf("Ndelta11 = %e, Ndelta13 = %e, Ndelta15 = %e, \n", Ndelta11, Ndelta13, Ndelta15);
		...
		Nphi11 = 0.5*tau_PSPG*C1*y23;
		Nphi12 = 0.5*tau_PSPG*C1*y31;
		Nphi13 = 0.5*tau_PSPG*C1*y12;

		Nphi21 = 0.5*tau_PSPG*C1*x32;
		Nphi22 = 0.5*tau_PSPG*C1*x13;
		Nphi23 = 0.5*tau_PSPG*C1*x21;

		Nphi31 = 0.5*tau_PSPG*C2*y23;
		Nphi32 = 0.5*tau_PSPG*C2*y31;
		Nphi33 = 0.5*tau_PSPG*C2*y12;

		Nphi41 = 0.5*tau_PSPG*C2*x32;
		Nphi42 = 0.5*tau_PSPG*C2*x13;
		Nphi43 = 0.5*tau_PSPG*C2*x21;

		Nphi51 = 0.5*tau_PSPG*C3*y23;
		Nphi52 = 0.5*tau_PSPG*C3*y31;
		Nphi53 = 0.5*tau_PSPG*C3*y12;

		Nphi61 = 0.5*tau_PSPG*C3*x32;
		Nphi62 = 0.5*tau_PSPG*C3*x13;
		Nphi63 = 0.5*tau_PSPG*C3*x21;

		//**********************Matrix Gphi*******************************
		Gphi11 = invrho*0.25*tau_PSPG*invArea*( y23*y23 + x32*x32 );
		Gphi12 = invrho*0.25*tau_PSPG*invArea*( y23*y31 + x32*x13 );
		Gphi13 = invrho*0.25*tau_PSPG*invArea*( y23*y12 + x32*x21 );
		
		Gphi22 = invrho*0.25*tau_PSPG*invArea*( y31*y31 + x13*x13 );
		Gphi23 = invrho*0.25*tau_PSPG*invArea*( y31*y12 + x13*x21 );
		
		Gphi33 = invrho*0.25*tau_PSPG*invArea*( y12*y12 + x21*x21 );	
		...
		//************************Matrix Npphi*******************************
		Npphi11 = tau_PSPG*sixth*( duxdx*y23 + duydx*x32 );
		Npphi12 = tau_PSPG*sixth*( duxdy*y23 + duydy*x32 );

		Npphi21 = tau_PSPG*sixth*( duxdx*y31 + duydx*x13 );
		Npphi22 = tau_PSPG*sixth*( duxdy*y31 + duydy*x13 );

		Npphi31 = tau_PSPG*sixth*( duxdx*y12 + duydx*x21 );
		Npphi32 = tau_PSPG*sixth*( duxdy*y12 + duydy*x21 );
		...
		fphi1 = tau_PSPG*Area*( y23*fxB + x32*fyB );
		fphi2 = tau_PSPG*Area*( y31*fxB + x13*fyB );
		fphi3 = tau_PSPG*Area*( y12*fxB + x21*fyB );
		...
		Nphiv[0] = 0.5*tau_PSPG*(( duxdx*ux + duxdy*uy )*y23 + ( duydx*ux + duydy*uy )*x32);
		Nphiv[1] = 0.5*tau_PSPG*(( duxdx*ux + duxdy*uy )*y31 + ( duydx*ux + duydy*uy )*x13);
		Nphiv[2] = 0.5*tau_PSPG*(( duxdx*ux + duxdy*uy )*y12 + ( duydx*ux + duydy*uy )*x21);

		Gphiv[0] = 0.5*tau_PSPG*invrho*( y23*dpdx + x32*dpdy); 
		Gphiv[1] = 0.5*tau_PSPG*invrho*( y31*dpdx + x13*dpdy);
		Gphiv[2] = 0.5*tau_PSPG*invrho*( y12*dpdx + x21*dpdy);
		...
		if(varglobal < 5000000){ //varglobal (<5 ISI, Ponto Fixo) (>=5 NI. Met. de Newton Inexato)
