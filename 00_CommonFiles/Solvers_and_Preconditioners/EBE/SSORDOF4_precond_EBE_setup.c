...
int SSORDOF4_precond_EBE_setup (ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs, int tag, double *F)
{
  ...
  if (tag==1){
		MatrixData->invDeaux = mycalloc("invDeaux of 'SSORDOF4_precond_EBE_setup'",NDOF*NNOEL*nel,sizeof(double));
		MatrixData->invDe = mycalloc("invDe of 'SSORDOF4_precond_EBE_setup'",nel,sizeof(double*));
		for (I=0; I<nel; I++)
  ...
  /* F preconditioning */
  double *faux = mycalloc("faux of SSORDOF4_precond_EBE_setup",(neq + 1), sizeof(double));
  for (I = 0; I < neq; I++){
    faux[I] = F[I];
  }
  faux[neq] = 0.0;
  ...
  SSORDOF4_precond_EBE (Parameters, MatrixData, FemStructs, faux, F);
