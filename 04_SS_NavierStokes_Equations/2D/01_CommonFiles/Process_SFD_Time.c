...
int Process(ParametersType *Parameters, MatrixDataType *MatrixData, FemStructsType *FemStructs, FemFunctionsType *FemFunctions, FemOtherFunctionsType *FemOtherFunctions)
{
	...
	double *s, *Fold, *u, *u_old, *F, *delta_old, *normres_old;//, *deltam_old;
	...
	sprintf(FileName,"../03_output/Residuo_%s_%s_%s_%s_%s_N%d_E%d.dat", Parameters->Experiments, Parameters->ProblemTitle, Parameters->StabilizationForm, Parameters->MatrixVectorProductScheme, Parameters->Preconditioner,Parameters->nnodes, Parameters->nel);
	...
	u_old = (double*) mycalloc("u_old of 'Process'", neq+1, sizeof(double));
...
//	int I, J;
	...
	while(delta2 > 0.000001 && it < itmax){
		it++;		
		iterold = iter;		
